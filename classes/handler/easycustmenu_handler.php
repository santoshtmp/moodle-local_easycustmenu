    ...
    public static function get_ecm_menu_items($type = 'navmenu', $contextlevel = 0, $courseid = 0, $roleids = [], $lang = '') {
        ...
        if ($roleids) {
            ...
            foreach ($roleids as $idx => $rid) {
                ...
                $pstart = 'rid_start_' . $idx;
                $pend   = 'rid_end_' . $idx;
                $pmid   = 'rid_mid_' . $idx;
                $roleConditions[] = "ecm.condition_roleid = :{$pexact}";
                $roleConditions[] = $DB->sql_like('ecm.condition_roleid', ":{$pstart}");
                $roleConditions[] = $DB->sql_like('ecm.condition_roleid', ":{$pend}");
                $roleConditions[] = $DB->sql_like('ecm.condition_roleid', ":{$pmid}");
                $sqlparams[$pexact] = (string)$rid;
                $sqlparams[$pstart] = $rid . ',%';
                $sqlparams[$pend]   = '%,' . $rid;
                $sqlparams[$pmid]   = '%,' . $rid . ',%';
            }
